...
Schematic gives you a real-time credit ledger with exactly-once event semantics, idempotent writes, and replay safety, so every action draws the balance down accurately and enforcement evaluates against current state. [Leases and reservations](/billing/credit-leases-and-reservations) let you reserve credits before an action and then commit or release them based on the outcome, so an in-flight agent can't overspend or get charged for work that failed. And pricing versioning keeps legacy customers on their old rates while new customers get the new ones, with no migration scramble.
...
- [Credit Leases and Reservations](/billing/credit-leases-and-reservations) — reserve, commit, and release so concurrent workers can't double-spend a balance.
