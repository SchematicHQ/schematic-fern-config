...
Completed purchases land as **Component Checkout** rows, each with the plan the customer came from and the plan they moved to. Entitlements are provisioned at that moment, so there is no window where someone has paid but cannot use what they bought. Find it on the company profile.
