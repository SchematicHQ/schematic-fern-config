...
description: Map Schematic companies, plans, add-ons, and prices onto their Stripe counterparts so the two-way sync avoids duplicates.
...
The end goal of setting up Stripe integration is that Schematic has the following values correctly linked so that Schematic can 2-way sync data with Stripe. This ensures that newly created records in either system are mirrored into the other and that no duplicate records are created. The mapped data is
...
2. Schematic Plan, Add-ons, and Prices ↔ Stripe product, Stripe price

### Schematic Plan, Add-on, and Price Mapping

Schematic Plans, Add-ons, and Prices are mapped to Stripe's Products and prices. 
...
2. Schematic Add-on → Stripe product + monthly/yearly/one-time price
...
<Info>
To understand more about `identify` and keys, see the [Developer Resources - Keys Management](/developer_resources/key_management) page.
</Info>
...
3. Going forward, Schematic will still prioritize `stripe_customer_id` as the key it uses to sync companies with Stripe.
