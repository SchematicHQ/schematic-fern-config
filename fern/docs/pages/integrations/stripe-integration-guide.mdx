...
To properly map companies between Stripe and Schematic, Schematic needs to have Stripe's `customer_id` added as a trait on each company. After the integration is setup, Schematic will do this automatically for newly created companies. The final state will look like this:
...
1. A one-time job to set `stripe_customer_id` and your existing `customer_key` on all companies
...
4. (uncommon) If you have a `customer_key` already stored in Stripe metadata, Schematic can import that along with the companies. 

<Info>
To understand more about `identify` and keys, see the [Developer Resources - Keys](/developer_resources/key) page.
</Info>

#### Pulling a `customer_key` out of Stripe metadata

If you have have a `customer_key` already stored in Stripe metadata, Schematic can import that along with the companies. After connecting Stripe, choose the 2nd option so that Schematic can pull the correct key from Stripe. 
