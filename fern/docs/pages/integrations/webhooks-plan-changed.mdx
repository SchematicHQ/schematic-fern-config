...
description: Read the action, cause, and plan fields on the company.plan_changed payload to tell upgrades, downgrades, and trial endings apart.

slug: integrations/webhooks/plan-changed
---

<Info>This page covers the `company.plan_changed` event in depth. For a full list of all webhook events, see [Webhooks](/integrations/webhooks).</Info>

The `company.plan_changed` webhook is one of the most important triggers for building sales and customer success workflows. It fires whenever a company's plan changes, whether an upgrade, downgrade, or the end of a trial period.
...
The `company.plan_changed` webhook is set up like all other webhooks in Schematic. Please follow the [general webhook setup instructions](/integrations/webhooks) to create an endpoint and select the `company.plan_changed` trigger.

## Fields

All Schematic webhooks are strongly typed in each of our [SDKs](/developer_resources/sdks/overview). 

[Here](https://github.com/SchematicHQ/schematic-node/blob/main/src/api/types/PlanChangeResponseData.ts) is that type for our node.js SDK for reference. 

A plan can change for many reasons, the most common ones are:
1. A company proactively changes its plan (either an upgrade or downgrade)
...
4. A company fails to pay its bills, and is moved to the fallback plan
...
| `"subscription_change"`         | The plan change resulted from a change in Stripe (e.g. non-payment, trial expiration)                      | Common                                                         |
...
In SDKs, this field is called `PlanChangeResponseDataBasePlanAction`, whereas in the raw webhook response, it can be found at `body.base_plan_action`.
...
| `subscribe`      | The company was moved from a free plan to a paid plan. This includes moving from a trial to a paid plan.                          |
| `unsubscribe`    | The company was moved from a paid plan to a free plan. This includes payment failure.                          |
...
<Info>For any of the `action = "checkout"` scenarios below, the same thing could be done via the Manage Plan modal in the Schematic App, at which point the `action` will be `manage_plan` instead of `checkout`.</Info>
...
| Customer switches from a free plan to a paid | `checkout` | `subscribe` | |
| Customer switches from a paid to a free plan | `checkout` | `unsubscribe` | |
...
| Customer is on trial and checkouts onto *any plan (even free!)* | `checkout` | `upgrade_trial` | |
| Customer missed payments and Stripe cancels subscription | `subscription_change` | | |
