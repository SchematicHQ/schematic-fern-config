...
**API key authentication returns a 401.** Confirm you are sending a secret key rather than a publishable one, and that the key belongs to the environment you mean to query. A production key pointed at sandbox data fails the same way an invalid key does.
