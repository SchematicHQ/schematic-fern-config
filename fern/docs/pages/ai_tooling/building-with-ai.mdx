...
Installing a client takes two steps that are easy to confuse. Adding the server writes a line of config, and your client will happily show the server in its list before it can call a single tool. Authenticating is what makes it work, and it is also where you make the two choices that decide what the assistant can see and do.
...
Both are fixed for the life of the connection. To change either, disconnect the server and connect again.
...
2. Start Claude Code and run `/mcp`. The server appears in the list marked as needing authentication. Select it and press Enter to open the browser flow, then sign in and choose your environment and access level.

3. Run `/mcp` again. The server now shows as connected, with the number of tools it exposes.
...
2. Start Gemini CLI and run `/mcp list`. The server is listed as disconnected, pending OAuth. Authenticate it:

```bash
/mcp auth schematic
```

3. Run `/mcp list` again to confirm the server is connected.
