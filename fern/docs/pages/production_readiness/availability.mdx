---
title: Availability
description: Schematic's uptime target and 24/7 support, plus built-in caching, Replicator, SDK fallbacks, and a self-hosted cache as ways to keep flag checks serving.
...
Live and historical uptime is published at [schematic.statuspage.io](https://schematic.statuspage.io/), broken out by API, App, Event processing, and Checkout Flow / Customer Portal. It is the fastest way to confirm whether something you are seeing is an incident on our side, and you can subscribe there to be notified when one is opened or resolved.

<Card title="Schematic Status Page" icon="fa-solid fa-signal" href="https://schematic.statuspage.io/">
  Current status, incident history, and notification signup.
</Card>

# Uptime, Support, and Backups

Schematic targets 99.99% uptime. This is an operational target, and the status page above is the public record of performance against it.

An uptime target is not the same commitment as a contractual service level. The standard online [Terms of Service](https://schematichq.com/terms) provide the Services on an as-is, as-available basis and do not include a service level agreement. Organizations that require a contractual uptime SLA with defined remedies should raise it during contracting.

The team is on call 24 hours a day, 7 days a week, and can be reached in your shared Slack channel or at `support@schematichq.com`. Incidents are opened on the status page, which supports notification subscriptions for opened and resolved incidents.
...
This model prioritizes simplicity and operational safety. If your application can tolerate brief periods of staleness, this is typically the best option, and it requires no additional configuration.
...
Replicator provides the same core architecture of local reads, real-time updates, and full cache coverage, but does so in a more robust, battle-tested, and operationally simple way.
