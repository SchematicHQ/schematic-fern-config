...
Schematic targets 99.99% uptime. That is the operational target we run the platform against, and the status page above is the public record of how we have done against it.

An uptime target and a contractual service level are different commitments, and a procurement review will ask which one it is getting. The standard online [Terms of Service](https://schematichq.com/terms) provide the Services on an as-is, as-available basis and do not carry a service level agreement. If your organization needs a contractual uptime SLA with defined remedies, raise it while contracting rather than assuming the published target is one.

The team is on call **24 hours a day, 7 days a week**, and can be reached in your shared Slack channel or at `support@schematichq.com`. Incidents are opened on the status page, where you can subscribe to be notified when one is opened or resolved.
