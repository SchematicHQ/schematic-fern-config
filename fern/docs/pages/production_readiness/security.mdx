...
Schematic acts as a processor for the Customer Data described above, and you remain the controller of it. The commitments that go with that split are in the [Data Processing Addendum](https://schematichq.com/dpa), which is available to every customer and covers the processing terms, the security measures, and the subject-rights assistance a controller needs from its processor.
...
Supporting the above, Schematic maintains a data inventory map, a completed Data Protection Impact Assessment, a documented pseudonymization procedure, and a sub-processor change notification process. GDPR controls are monitored continuously in Vanta alongside the SOC 2 program. Current status is on the [Trust Center](https://trust.schematichq.com), and the underlying documents can be provided on request.
