...
You can add a usage event corresponding to when a feature is used in your product. When you send track events to Schematic using the [/events endpoint](/api-reference/events/create-event), we will handle attribution to the respective company and feature, and reflect usage in the Feature Usage tab.
...
When you send track events to Schematic using that event name, they will be associated with the feature.