...
- **Boolean** features need nothing configured beyond the feature and its flag key.
- **Event-based** features use the **Event** field, a search-or-create box. Type the event name you intend to send, for example `api_calls`, and pick **Create event: api_calls** from the dropdown. That name is stored on the feature. Usage starts accruing the first time your application calls `track` with the matching event name, and the feature needs no change at that point.
- **Trait-based** features use the **Trait** field the same way. Type the trait name, for example `seats`, and choose **Create trait**. This creates the trait definition immediately, ready to receive values once you start reporting them via `upsertCompanyTrait`, `upsertCompany`, or `identify`.
