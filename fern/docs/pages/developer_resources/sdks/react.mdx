---
title: React
description: Use schematic-react to quickly identify users, track events, and check flags client-side in your react app with Schematic.
...
If a [usage warning](/feature-management/usage-warnings) is configured on the entitlement, the hook also returns `warningTiers`, so you can warn a customer before they hit a limit rather than after. Each tier is a `{ key, value }` pair in the entitlement's usage units, and the dashboard writes a single tier under the key `default`:
...
If you're building with Next.js, we recommend reading our [Next.js SDK guide](/developer_resources/sdks/nextjs) for more detailed recommendations on how implement Schematic in your Next.js application.
