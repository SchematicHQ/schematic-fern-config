---
title: Angular
description: Use schematic-angular to quickly identify users, track events, and check flags client-side in your angular app with Schematic.
slug: developer_resources/sdks/angular
---
...
_Note: `isPending$` checks if entitlement data has been loaded, typically via `identify`. It should be used to wrap flag and entitlement checks, but never the initial call to `identify`._

### Usage warnings

If a [usage warning](/feature-management/usage-warnings) is configured on the entitlement, the emitted entitlement also carries `warningTiers`, so you can warn a customer before they hit a limit rather than after. Each tier is a `{ key, value }` pair in the entitlement's usage units, and the dashboard writes a single tier under the key `default`:

```typescript
approachingLimit$ = this.schematic.entitlement$("my-flag-key").pipe(
  map((entitlement) => {
...
It surfaces the `settled` balance, which is the spendable number you should show end users. For advanced lease-aware accounting, read the underlying client instead: `getClient().getCreditBalance(creditId)` returns all three views of the balance as a point-in-time snapshot: `settled`, `remaining` (excluding open lease holds), and `reserved` (the amount held by open leases). Subscribe with `getClient().addCreditBalanceListener(callback)` to react to changes. The credit ID is available on a feature's entitlement: `entitlement$(key)` emits `creditId` for credit-based features.
