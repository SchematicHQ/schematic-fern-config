...
description: Use schematic-js to quickly identify users, track events, and check flags client-side in your JavaScript app with Schematic.
...
If a [usage warning](/feature-management/usage-warnings) is configured on the entitlement, `getFlagCheck` returns it as `warningTiers`, so you can warn a customer before they hit a limit rather than after. Each tier is a `{ key, value }` pair in the entitlement's usage units, and the dashboard writes a single tier under the key `default`:
