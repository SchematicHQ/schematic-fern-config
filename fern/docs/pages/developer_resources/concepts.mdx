...
A company is always on a specific **version** of its plan. Plans are versioned (`draft` → `published` → `archived`), and when you publish a new version you choose whether to migrate the company to it or leave it on its current version. Add ons are versioned independently. See [Plan Versions](/catalog/plans#plan-versions) for the full model.
