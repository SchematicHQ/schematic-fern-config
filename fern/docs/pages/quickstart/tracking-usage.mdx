...
2. In the **Monetization Settings** panel, set the **Type** to Numerical and the **Value** to 25 -- this is how many events a company can use per period.
3. Under **Define reset**, set the **Period** to "Monthly" and have the usage reset "According to billing period" (the most common setting). This means a company gets 25 usages per month, measured from the day they signed up (e.g. if they sign up on March 15th, the "month" runs March 15th to April 14th).
