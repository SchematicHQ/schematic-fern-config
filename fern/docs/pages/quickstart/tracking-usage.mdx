...
Schematic tracks the most recent `track` or `identify` (see [tracking usage](/quickstart/identify-users)) event for a company and user. This is helpful while getting started to make sure your events are being sent to Schematic. Once implemented, this is also helpful for understanding usage patterns and customer health. 
