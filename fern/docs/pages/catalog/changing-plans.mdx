...
When a company changes plans, whether an upgrade or a downgrade, Schematic automically updates handles the company's entitlements, usage limits, and billing status in Stripe (if applicable). Plan changes can occur from a few different sources -- in all cases, the steps Schematic takes are the same. The possible reason for a plan change are:
...
Stripe-linked plnas have additional considerations concerning how usage based charges are synced to Stripe. 
