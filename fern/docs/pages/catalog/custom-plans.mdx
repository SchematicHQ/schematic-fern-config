...
<Note>
This is different from marking a standard plan as "Custom" to show a "Talk to us" message instead of a price in your pricing table, which is a display setting covered in [Configuring the Catalog](/catalog/configuration#custom-plans). A custom plan here is a real, per-company plan with its own entitlements and billing.
</Note>
...
The activation strategy is **On payment**: the company becomes active on the custom plan once the invoice is paid, within the configured due window (for example, 30 days). Until then, the plan stays pending.
