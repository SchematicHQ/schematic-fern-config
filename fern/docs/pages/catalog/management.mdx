...
You can manage a company's plan directly on the company profile page in Schematic using the **Manage Plan** button. These changes automatically sync with Stripe to update a company's subscsription. The company's current plan version is shown both on the profile (e.g. a "v3" badge) and in the Manage Plan panel; assigning a plan here uses its latest published version. 
