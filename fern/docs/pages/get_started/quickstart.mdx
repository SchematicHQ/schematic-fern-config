...
We’ll opt for the second option (using the Stripe customer ID). Once connected, Schematic should sync companies, subscriptions, and products from Stripe and you should see them appear in the Schematic Dashboard.
...
3. In the Billing step, link the Schematic Plan to a Stripe Product
    1. Select the Stripe Product that reflects the Schematic Plan. Companies that have active subscriptions will automatically become a member of the Schematic Plan.
...
After [configuring your Catalog](https://docs.schematichq.com/components/overview), do the following:
...
<Info>Once you configure a Component to your liking, you can drop it directly into your application.</Info>
