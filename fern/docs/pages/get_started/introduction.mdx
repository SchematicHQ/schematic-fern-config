...
## Does Schematic Replace Stripe?

No, Schematic doesn't replace Stripe. Instead, Schematic extends Stripe into your product so you can enforce plan features and usage limits automatically. Schematic fully manages its Stripe integration on your behalf, bi-directionally syncing company and usage data to Stripe (no more painful Stripe webhooks!). You don't need to build or operate your own Stripe webhook handlers to keep billing state, entitlements, and usage in sync. 

Most users will never need to interact with Stripe directly, instead using Schematic's Checkout and Customer Portal components in their product and Schematic app for their customer management needs. However, for advanced use cases, like 3rd party tax providers or custom invoices, you can always fall back to Stripe.
