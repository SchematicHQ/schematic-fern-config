...
A billing entity lets one company pay for another company's subscription. The company that receives the plan keeps its own Schematic record, its own entitlements, and its own usage data. Only the bill moves.
...
- **Multiple products under one customer.** You launch a second product that is distinct enough to warrant its own plans and features, so it gets its own company record while the original company continues to pay.
...
{/* SCREENSHOT NEEDED: Stripe subscription detail page for the Acme Corporation subscription (owned by the Advanced Analytics Ltd customer), with the Metadata panel in the right sidebar visible showing schematic_billing_entity_id and schematic_company_id. Save as ../../assets/images/billing/billing-entity-stripe-metadata.png */}
