...
slug: billing/when-credits-run-out
---

Every credit grant on a plan or add-on carries a **When credits run out** setting, and it decides what a customer hits at a zero balance. Pick the wrong one and you either cut off an enterprise account mid-workload or hand a self-serve customer a bill they never agreed to. The four options trade continuity against control over spend:
...
[Credit bundles](#credit-bundles) sit alongside all four. A company buys a bundle as a one-time purchase, and whether it can depends on the bundle, not on this setting.

You'll find the setting in the credit grant form: open a plan or add-on in the Catalog, click "Add credit grant" or an existing grant, and look for **When credits run out** below the reset and expiration fields.
