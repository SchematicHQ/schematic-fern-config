...
Schematic has two kinds of hold, and the shape of the work decides which one you want.
...
| Coordinated by | The Node SDK, through a cache shared across processes | The server, on each reserve |

A lease is a batching optimization for high-throughput local gating. The Node SDK acquires one tranche of credits, carves each request's share out of it locally, and decides a check without an API round trip.

A reservation is a server-side hold for one operation. You take it, run the work, and settle it with the usage you saw. Reach for it when an operation takes seconds rather than milliseconds, such as an external lookup or an inference call, when you are calling from a language other than Node, or when the deployment cannot run DataStream.

The rest of this page describes leases except where it says otherwise. [Reservations](#reservations) covers the per-operation hold.
...
An agent reads the hold's unspent portion as `granted_amount` less `tracked_amount` on the reserve or extend response, as `creditReserved` on the entitlement a check returns, or as the reserved value on the company's credit balance, and extends by the shortfall when its remaining work exceeds it. The Node SDK does this on its own, extending in the background once the lease's unspent balance falls below a configured low-water mark.
...
A lease is the wrong object for a per-operation hold, and the failure is not visible until a second operation for the same company and credit type overlaps the first. Then it breaks from both ends: the second acquire returns the existing lease rather than adding to the hold, so the second operation is not covered by anything, and the first release closes the lease under the operation still running. Use a [reservation](#reservations) for a per-operation hold. Each one is its own hold, and several can be open for the same company and credit type at once.
...
The debit happens under a row lock, so two operations racing for the last of a balance see exactly one succeed. A balance that cannot cover the full amount returns a `402` and consumes nothing. There is no partial hold, which is the opposite of how a lease behaves; see [Partial funding](#partial-funding). Send an `idempotency_key` to make retries safe. The same key returns the original reservation, and the same key with a different company, credit type, or amount returns a `409`.
...
Settle by sending the track event you would have sent anyway, with `reservation_id` set to the reservation's id. The first track carrying the id closes the reservation.

- What the operation used is kept.
- The unspent remainder returns to the grants it was drawn from.
- Usage beyond the hold is billed as overage, which can push the balance negative. Usage is never dropped.
...
A balance that cannot fund the hold comes back as a `200` whose `value` is false, with the reason `Insufficient credits`, rather than the `402` the reservations endpoint returns. A flag that denies, a feature that is not credit metered, and a credit whose ledger lives on an external billing provider all return the plain check result with no `reservation`, and you track usage as usual. The check is logged like any other.

The endpoint takes a secret API key only, because a publishable key must not be able to draw down a balance.

## Using an SDK

The Node SDK covers both kinds of hold behind `client.check()` and `client.trackWithReservation()`, in two modes.

**Client mode** maintains one lease per company and credit type and issues per-request reservations against that lease locally, so an individual check does not require an API round trip, and reservations are gated across processes through a shared cache. It needs DataStream and runs on Node only. This is the mode for gating high volumes of inference calls or agent runs.

**Server mode** makes one check-and-reserve call per check and settles it with a track event. It needs neither DataStream nor Redis.

`creditLeases.mode` selects between them. The default is `auto`, which picks client mode when DataStream is enabled and server mode otherwise. The Node SDK reference covers configuration, failure behavior, and pre-warming in detail; see [Credit Leases and Reservations](/developer_resources/sdks/nodejs#credit-leases-and-reservations).

From the other SDKs, call the endpoints in [Using the API](#using-the-api) yourself. For a per-operation hold that is [check and reserve](#checking-and-reserving-in-one-call), or a reservation and a settling track event.

## Technical details

In client mode the Node SDK keeps one lease per company and credit type and carves per-request reservations out of it, so what is worth tuning is how large the lease is, how long it and its reservations live, and when it is topped up. The settings below are the Node SDK's.
...
`amount` must be positive, and the full amount is held or nothing is. `expires_at` is optional, defaults to one minute out, and may be at most one hour out. The response carries the reservation's `id`, its `reserved_amount`, the `settled_amount` recorded so far, and `released_at`, which is null while the hold is open.
...
To gate on a flag and take the hold together, [Check and reserve flag](/api-reference/features/check-and-reserve-flag) accepts the body a check accepts plus `quantity`, `expires_at`, and `preflight`. `quantity` defaults to 1, and the hold is that many times the entitlement's consumption rate, unless `preflight.credit_cost` names the entitlement's credit, in which case that cost is held instead.
