...
2. Click "Add credit grant" to create a new credit grant. *NOTE: Auto top-ups can only be configured for new credit grants.*
3. Select the Credit Type and Credit Amount you want to use for the auto top-up. *NOTE: only one credit type can be added to a plan. If you are trying to add auto top-ups to an existing plan, you may need to remove the existing credit grant and create a new one.*
4. Enable "Auto Top-Up".

![Auto Top-Up](../../assets/images/billing/burndown-auto-top-up.png)

4. The "Remaining balance threshold" is the percentage of the credit grant that will trigger the auto top-up. We've used 10% for this example.
...
6. The "Top-up Grant Expiration" represents the expiration policy for the auto top-up grant. We recommend a 1 year duration for the auto top-up grant -- this provides customers with adequate time ot use the credits, but also simplifies revenue recognition.
...
Auto top-ups are triggered when processing an event that draws from a credit balance and the customer's balance is below the remaining balance threshold. Schematic ensures that if multiple events trigger in a short period of time, only one auto top-up will be purchased. 
