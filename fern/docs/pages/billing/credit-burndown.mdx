...
Credits that expire sooner are spent first, so a bundle carrying an expiry window is drawn before a plan grant that never expires. Among grants that never expire, the oldest goes first, so these bundles are only drawn from once the period's plan credits are exhausted. See [Consumption order](#consumption-order) for the full picture.
...
3. **Everything else, soonest to expire first**, which in practice means plan grants, purchased bundles, and promotional credits ordered by expiry date

Grants that never expire go last, and grants sharing an expiry date are drawn oldest first. Spending the shortest-dated credits first is what keeps a customer from letting a 30-day bundle lapse while a 12-month one sits ahead of it.
...
When a plan grants both per-license and per-company credits, as described in [Seat and Agent Budgets](/billing/seat-and-agent-budgets), the per-license credits are spent first.
...
| When credits run out | `auto_topup_availability`, `postpaid_enabled` | See [API fields](/billing/when-credits-run-out#api-fields) |
...
| Compatible Plans | `compatible_plan_ids`. Empty means every plan |
...
<Warning>
The **Burndown strategy** field on the credit type shows the strategy the credit type is using, but the dashboard cannot change it. Set it through the API.

`burn_strategy` accepts `expiration_priority` (the default), `first_in_first_out`, `last_in_first_out`, and `plan_first_then_credit_bundles_first_in_first_out`. The [Consumption order](#consumption-order) above describes the default. Adjustment and rollover grants are consumed first under every strategy.
</Warning>
...
4. Set **When credits run out** to "Automatic top-ups" or "User-controlled top-ups", covered in [Who controls auto top-up](#who-controls-auto-top-up) below.

![Auto Top-Up](../../assets/images/billing/burndown-auto-top-up.png)

5. **Top-up Threshold** is the balance that triggers a purchase, given as a number of credits remaining rather than a percentage. The balance it checks counts all plan grants, purchased grants, and promotional grants.
...
7. **Top-up Grant Expiration** sets the expiry policy for the purchased credits. Auto top-up grants offer Duration and No Expiry only, not the billing period options. A 1 year duration is a good default, long enough for customers to use the credits and short enough to keep revenue recognition simple.
...
**When credits run out** decides both whether auto top-up runs and who operates it. Two of its four options are top-up policies:

| Setting | What happens |
| --- | --- |
| Automatic top-ups | Companies on this plan grant receive a top-up automatically when they reach the threshold. Buyers cannot change it. |
| User-controlled top-ups | Companies turn auto top-up on themselves in checkout and through components. Your threshold and amount become their starting defaults. |

The other two, Block usage and Bill in arrears, are covered with the top-up policies in [When Credits Run Out](/billing/when-credits-run-out).
...
One-time [bundle](#5-configure-credit-bundles) purchases are set on each bundle rather than on the grant. A bundle's **Plan Compatibility Settings** choose which plans can buy it, and a company can buy any live bundle compatible with its plan regardless of the top-up policy. See [Credit bundles](/billing/when-credits-run-out#credit-bundles).

### Per-company exceptions

Top-up settings live on the plan grant, so every company on that plan starts with the same threshold and amount. Individual companies can be given different values through the [API](/api-reference/billing-credits/list-company-grants), which is currently the only way to do it, as the dashboard has no per-company top-up control.

Under User-controlled top-ups the customer changes their own threshold and amount, which covers many of the cases an override would otherwise handle.
...
Three webhook events report the outcome of each attempt: `auto.topup.success`, `auto.topup.hard.failure`, and `auto.topup.retry.exceeded`. A hard failure carries the Stripe error code when the failure originated there, which is what lets you tell a declined card apart from a missing payment method. Subscribe to them under Settings > Integrations > Webhooks, and see [Entitlement & Credit Trigger Webhooks](/integrations/webhooks/entitlement-triggers#auto-topup-events) for the full payloads.
