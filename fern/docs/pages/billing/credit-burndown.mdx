...
Customers who already subscribed reach the same bundles from a **Buy More** button next to each credit balance in the customer portal, which opens checkout on the credits step and leaves their plan alone. That button belongs to the [Metered Features](/components/element-library#metered-features) element, so add that element to the portal component your customers see.
...
4. Set **Top-up Availability** to Automatic or User-controlled, covered in [Who controls auto top-up](#who-controls-auto-top-up) below.
...
| --- | --- |
| Off | Auto top-up never runs and companies cannot turn it on. Existing credits remain until they expire. |
| Automatic | Companies on this plan grant receive a top-up automatically when they reach the threshold. Buyers cannot change it. |
| User-controlled | Companies turn auto top-up on themselves and buy more credits in checkout and through components. Your threshold and amount become their starting defaults. |
...
One-time [bundle](#5-configure-credit-bundles) purchases run off a per-bundle setting instead of a grant setting. Each bundle in the catalog's Credit Bundles section carries a **plan compatibility** dropdown, and an empty selection lets every plan buy the bundle. Edits there save immediately, for every catalog the bundle is live in. Top-up availability does not enter into it, so you can let a customer buy credits on demand without letting them set up an automatic refill, or the reverse.

A company on a plan outside a bundle's compatibility list gets no buy-more button for that bundle, and a purchase request naming it is rejected. The dropdown lists your standard plans, so leave compatibility unrestricted on any bundle you want a custom-plan customer to buy.
