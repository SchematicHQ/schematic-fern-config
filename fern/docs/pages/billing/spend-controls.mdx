...
The controls above apply to a company as a whole. Draws are also attributed to the user or agent that caused them, and credit grants can scale with the number of licenses a company holds. See [Seat and Agent Budgets](/billing/seat-and-agent-budgets), which covers per-actor reporting and what is not enforced per actor yet.
