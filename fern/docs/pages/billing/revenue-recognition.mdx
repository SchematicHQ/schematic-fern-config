...
What they need from engineering is measurement: a record of every grant, draw, reservation, and expiry, with its price and timestamp attached, at a granularity their policy can be applied to. Schematic's credit ledger is that record. This page summarizes what ASC 606 asks for, documents what the ledger contains, states where the boundary with Stripe and your accounting system sits, and shows how to read the data out.
...
Usage entries also carry a `usage_reason`. Ordinary consumption is `track`. Where [credit leases and reservations](/billing/credit-leases-and-reservations) are in use, `lease_hold`, `lease_release`, `reservation_hold`, and `reservation_release` entries record credits reserved and returned. A reservation is an authorization rather than delivery, so credits reserved and released without being consumed are not revenue. Counting them as consumption recognizes revenue against work that was never performed.
...
| `settled` | The balance net of actual consumption, excluding reserved credits |
...
- [Credit Leases and Reservations](/billing/credit-leases-and-reservations) - why a reserved credit is not a delivered one.
