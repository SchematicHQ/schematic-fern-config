...
| Coordinated by | The backend SDK, through a cache shared across processes | The server, on each reserve |
...
Each SDK exposes a credit lease mode setting. The default is `auto`, which picks client mode when DataStream is enabled and server mode otherwise. Each [SDK reference](/developer_resources/sdks/overview) covers configuration, failure behavior, and pre-warming.

Without an SDK, call the endpoints in [Using the API](#using-the-api) directly. For a single operation, use [check and reserve](#checking-and-reserving-in-one-call), or take a reservation and settle it with a track event.
...
To check a flag and reserve credits in one call, [Check and reserve flag](/api-reference/features/check-and-reserve-flag) accepts the body a check accepts plus `quantity`, `expires_at`, and `preflight`. The reserved amount is the entitlement's consumption rate times the units the request states: `quantity` when it is given, otherwise `preflight.event_usage.quantity` when its `event_subtype` is the entitlement's, otherwise `preflight.usage`, otherwise 1. `preflight.credit_cost` takes precedence over all of those: when it names the entitlement's credit, that cost is reserved as given. Pass an `idempotency_key` to make the call safe to retry: a second request with the same key returns the reservation the first one took instead of reserving again, even when the first hold has since lowered the balance.
