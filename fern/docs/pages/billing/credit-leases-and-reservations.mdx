...
The Node and Python SDKs support both leases and reservations through their check and track-with-reservation methods, in two modes. The other backend SDKs call the endpoints directly. Frontend SDKs do not take leases or reservations, because a publishable key cannot draw down a balance.
...
Both SDKs expose a credit lease mode setting, `creditLeases.mode` in Node and `credit_leases.mode` in Python. The default is `auto`, which picks client mode when DataStream is enabled and server mode otherwise. Both SDK references cover configuration, failure behavior, and pre-warming; see [Credit Leases and Reservations](/developer_resources/sdks/nodejs#credit-leases-and-reservations) and [Credit reservations](/developer_resources/sdks/python#credit-reservations).

From the other SDKs, or without one, call the endpoints in [Using the API](#using-the-api) directly. For a single operation, use [check and reserve](#checking-and-reserving-in-one-call), or take a reservation and settle it with a track event.
