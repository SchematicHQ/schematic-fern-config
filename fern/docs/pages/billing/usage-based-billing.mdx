...
6. Next, select how the duration of the period to track usage. Montly is by far the most common, but you can also choose daily or yearly.
7. If monthly is selected (as is done here), you will need to determine if the period should reset at the beginning of the month or based off of the billing period of the subscription (i.e. if the customer signed up on the 15th, the period would be measured 15th-14th of next month). Basing it off of the billing period is the most common choice. 
...
9. Finally, a hard limit can be set (this is optional, but can be helpful if there are resource limits you're trying to manage or you want to force conversion into a higher plan at a specific usage threshold). At this limit, Schematic will prevent the customer from using more.
