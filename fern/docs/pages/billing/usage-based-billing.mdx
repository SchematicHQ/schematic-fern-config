...
description: Configure pay as you go, pay in advance, overage, volume, and graduated pricing on an individual entitlement.
...
6. Next, select the duration of the period to track usage. Monthly is by far the most common, and Schematic also supports daily and weekly.
...
Because the customer had usage in 2 different tiers, they paid the flat fee price for both tiers. This is typically not the desired behavior, which is why flat fee pricing is not common in graduated pricing models. If you want to include a flat fee, consider adding it to the base plan price, or only including it in the first tier.
...
2. Click on the entitlement that you'd like to monetize.
3. For Type, select "Tiered".
4. Then, a second "Type" option will appear below and select "Graduated".
...
A customer on pay as you go or overage can run up a large balance between invoices, and you carry that
usage unpaid until the period closes. A billing threshold shortens the wait. Set one on an entitlement
and Stripe issues an invoice as soon as the customer's metered usage reaches the number of units you
name, then bills the remaining usage at the end of the period as usual.

### Configuring a billing threshold

1. Navigate to a plan with a metered entitlement priced as "Pay as you go" or "Overage".
2. Click on the entitlement that you'd like to monetize.
3. Turn on "Billing threshold".
4. Enter the "Usage threshold (units)", the amount of usage that triggers the early invoice. The minimum is 50 units.

Below the field, Schematic converts the units into money at the unit price you have already configured,
so you can confirm that 10,000 units at \$0.02 comes to \$200 of usage before you save. In Stripe, the
value lands on the subscription item as a usage billing threshold.

### What the customer sees

Your buyer sees the threshold before they pay, which matters when your terms of service describe when
you invoice. On the line item for that feature, in both checkout and the [pricing table](/components/element-library)
component, Schematic renders an info icon next to the price. The customer hovers it to read the
threshold that triggers an early invoice and to see that the rest is billed monthly.
...
### Behavior by billing model

Negative quantities affect billing differently depending on the pricing model configured for the feature's entitlement:

| Billing model | Effect of negative quantities |
|---|---|
| **Pay as you go** | Negative usage is reported to Stripe. Stripe meters accept negative values, but the total usage for a billing period [cannot go below zero](https://docs.stripe.com/billing/subscriptions/usage-based/meters/configure#fix-incorrect-usage). |
| **Volume pricing** | Negative usage reduces the total quantity reported to Stripe, potentially moving the customer into a lower pricing tier. Period usage cannot go below zero. |
| **Graduated pricing** | Negative usage reduces the total quantity reported to Stripe. Period usage cannot go below zero. |
| **Fixed fee with overage** | Negative usage reduces the total quantity reported to Stripe, which may reduce overage charges. Period usage cannot go below zero. |
| **Credit burndown** | Negative usage triggers a **credit refund** — credits are returned to the company's balance via an adjustment grant. See [Credit Refunds via Negative Quantities](/billing/credit-burndown#credit-refunds-via-negative-quantities) for details. |
