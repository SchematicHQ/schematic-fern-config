...
Beyond this documentaiton, the Pricing Table source can be found [here](https://github.com/SchematicHQ/schematic-js/blob/main/components/src/components/elements/pricing-table/PricingTable.tsx)
...
| --- | --- | --- | --- |
| callToActionUrl | URL for plans without a price but a CTA like "Contact Sales" | `undefined` | If this is unset, plans without a price won't display a CTA |
| onCallToAction | Callback function to execute when a CTA is clicked | `undefined` | If `callToActionUrl` isn't set, this field will have no affect |
...
Below are the options that can be set on the Pricing Table to control it's functionality. These settings should all be set on the `PricingTable` component itself (example below).
...
| plans.showInclusionText | Show "What's included" text | true | |
...
Additionally, all of the typography options are configurable. The any text element can have it's typography set to any of the following `FontStyle` options:
