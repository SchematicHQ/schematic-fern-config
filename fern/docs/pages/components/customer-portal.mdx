...
description: See what Schematic checkout handles beyond Stripe Checkout, how it syncs subscriptions, and which lifecycle events it emits.
...
The checkout flow itself walks customers through up to five steps:
...
## Topping Up Credits Mid-Subscription

A customer who burns through their plan's credit allowance halfway into the month buys more from the portal itself, without changing plans. Each credit on the [Metered Features](/components/element-library#metered-features) Element carries a **Buy More** button beside its usage bar, and clicking it opens checkout on the credits step. The customer picks a bundle, confirms, and their balance goes up. A card already on file removes the payment step as well.

The button belongs to Metered Features specifically. Add that element to your portal in the builder, since a portal that shows entitlements through Included Features alone has nowhere to put it. [Element Library](/components/element-library#metered-features) lists the catalog and configuration conditions that decide whether the button renders, and [Credit Burndown](/billing/credit-burndown) covers setting up the bundles it sells.

Customers on custom plans buy the same way. Because bundles can restrict themselves to particular plans and that restriction names your standard plans, leave a bundle's compatible plans empty when custom-plan customers should be able to buy it.
...
Schematic ensures that customer billing state stays in sync between Stripe and Schematic.
...
**Live plans.** Within Schematic, you control which plans are available for customers to upgrade or downgrade to in Products > Configuration. Only plans mapped to Stripe products can be added as live plans.
...
**Tax collection.** Schematic supports Stripe Tax to collect billing address information and calculate taxes at checkout. This is configured under Products > Configuration. See [Advanced Usage](/components/advanced-usage) for details.
