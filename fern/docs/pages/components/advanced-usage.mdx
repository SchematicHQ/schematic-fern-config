...
There are 2 additional ways you can control the component state when programmatically launching. These are unlikely to be needed in most cases, but are documented here for completeness.
...
In addition to the checkout flow, you can also choose to load other parts of the Schematic Components library instead. It is unlikely you will need to do this, but it is documented here for completeness.

To do this, you'll want to set a different value for `layout`. The accepted values for `layout` are: 
...
| `unsubscribe` | The unsubscribe flow. This is normally handled by adding an Unbuscribe button to your portal using the component builder. |
