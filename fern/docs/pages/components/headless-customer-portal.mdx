...
description: Replace the portal component with your own UI using the hydrate, invoices, balance, and payment method endpoints.
...
<Note>This guide only covers building your own customer portal. You will still need to use Schematic's Checkout Component to handle checkout. See our [launching checkout programmatically guide](/components/advanced-usage#launch-checkout-programmatically) for more info</Note>
...
**Reset periods.** `period` is the window usage is measured over: `current_day`, `current_week`, `current_month`, or `all_time`. `metric_reset_at` is the next reset timestamp, and for a `current_month` period `month_reset` tells you whether the month follows the company's billing cycle (`billing_cycle`) or the calendar (`first_of_month`).
