...
`locale` takes a BCP 47 tag such as `it-IT` or `ja-JP`. A region tag uses its base language's bundle, so `it-IT` reads from the `it` bundle. The components re-render when `locale` changes.
