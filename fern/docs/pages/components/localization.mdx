...
- Make sure to keep the `{{placeholders}}` in your translation. The components fill in plan names, dates, prices, and quantities at render time, and you can move a placeholder anywhere your language's word order needs. Translate from each value rather than its key, since some keys are shorthand that leave the placeholders out. `"Access to plan will end."` displays as `"Access to {{plan}} will end on {{date}}."`, so your translation needs both `{{plan}}` and `{{date}}`.
