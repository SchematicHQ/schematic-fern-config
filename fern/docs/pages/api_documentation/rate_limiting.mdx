---
title: Rate limiting
description: Rate limits for the Schematic API, split between the low-latency application hot path and the core resource management tier.
...
Users should treat these limits as maximums and should avoid designing systems that generate unnecessary load. To request an increased rate limit, please contact support so we can understand the use case.
...
2. **Concurrent flag checks.** Often, this occurs when flag values are retrieved synchronously. We suggest using our bulk flag check endpoint to retrieve flag values asynchronously and caching those results locally.
